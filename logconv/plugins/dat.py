...
class DatPlugin(LogFormatPlugin):
    name = "dat"
    extensions = [".dat"]
    ...
    def detect(self, path: str, sample: bytes = None) -> bool:
        return bool(path and path.lower().endswith(".dat"))

    def parse(self, path, options=None) -> LogDocument:
        meta = {"path": path, "format": "dat"}
        try:
    ...
    def write(self, path, log_doc: LogDocument, options=None) -> WriteResult:
        try:
            ...
            with open(path, "wb") as f:
                f.write(raw_bytes)
            warnings = ["DAT plugin performs passthrough copy; structure not decoded."]
